...
void gated_delta_net_sycl(const float *     q,
    ...
    float * state = (dst + attn_score_elems) + state_out_offset;
...
#pragma unroll
        ...
        if constexpr (keep_rs_t) {
            ...
            if (target_slot >= 0 && target_slot < K) {
                float * curr_state_o = (dst + attn_score_elems) + target_slot * state_size_per_token + state_out_offset;
#pragma unroll
                for (int r = 0; r < rows_per_lane; r++) {
                    const int i = r * warp_size + lane;
                    curr_state_o[col * S_v + i] = s_shard[r];
                }
            }
        }
    }
