...
static bool ggml_backend_opencl_buffer_cpy_tensor(ggml_backend_buffer_t dst_buf, const ggml_tensor * src, struct ggml_tensor * dst) {
    // Copy tensor data from src (any buffer) into dst (in this OpenCL buffer)
    // Used by the graph scheduler for cross-backend tensor copies
    ggml_backend_opencl_context *backend_ctx = ggml_cl2_init(dst_buf->buft->device);
    cl_command_queue queue = backend_ctx->queue;

    size_t size = ggml_nbytes(src);

    // Read source data to temporary host buffer
    std::vector<uint8_t> tmp(size);
    if (src->buffer && src->buffer->iface.get_tensor) {
        src->buffer->iface.get_tensor(src->buffer, src, tmp.data(), 0, size);
    } else if (src->data) {
        memcpy(tmp.data(), src->data, size);
    } else {
        return false;
    }

    // Get the destination OpenCL buffer and offset
    ggml_backend_opencl_buffer_context * ctx = (ggml_backend_opencl_buffer_context *) dst_buf->context;
    ggml_tensor_extra_cl * dst_extra = (ggml_tensor_extra_cl *)dst->extra;
    if (!dst_extra) return false;

    cl_int err;
    CL_CHECK(clEnqueueWriteBuffer(
        queue, ctx->buffer[0], CL_TRUE,
        dst_extra->offset, size, tmp.data(),
        0, NULL, NULL));
